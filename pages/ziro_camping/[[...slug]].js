...
import campgroundrule from "../../schema/campgroundrule.json";
import campingessentials from "../../schema/campingessentials.json";
import faqrule from "../../schema/faq.json";
import highlights from "../../schema/highlights.json";
import OfferCatalog from "../../schema/OfferCatalog.json";
...
const Ziro_camping = ({ data, base_url, faqdata }) => {
   ...
   const insertJsonLd = (json) => {
    ...
    useEffect(() => {
    [campgroundrule, campingessentials, faqrule, highlights, OfferCatalog].forEach((json) => {
        insertJsonLd(json);
    });
...
export async function getServerSideProps(context) {
    ...
    const otherPackages = [
        {
            slug: "3N-4D",
            duration: "3N/4D",
            label: "Ziro Festival Camping Package 3N/4D",
            price: 14999
        },
        {
            slug: "2N-3D",
            duration: "2N/3D",
            label: "Ziro Festival Camping Package 2N/3D",
            price: 11999
        },
        {
            slug: "", // base /ziro_camping
            duration: "4N/5D",
            label: "Ziro Festival Camping Package 4N/5D",
            link: "/ziro_camping",
            price: 17999,
        },
    ];
    const additionalTrips = otherPackages
        .filter((pkg) => pkg.slug !== currentSlug)
        .map((pkg) => ({
            name: pkg.label,
            link: `/ziro_camping/${pkg.slug}`,
            price: pkg.price
        }));

    const data = {
        name: "Ziro Music Festival Camping Packages",
        title: "Ziro Music Festival Camping Packages",
        ...
        meta: {
            title: "Ziro Music Festival Camping 2025 | Things to Do & Stay",
            description: "Experience Ziro Music Festival camping like never before. Enjoy soulful music, starlit nights, and a truly unforgettable festival escape. Book your spot now.",
    ...
    if (slug?.[0] === "3N-4D") {
        data.durationtext = "4 Days & 3 Nights";
        data.tripdetails = "3N/4D Twin Sharing Dome Tent Stay + Complimentary Breakfast + Campsite Facilities";
        data.checkindate = "26-09-2025";
        data.price = 14999;
        data.p_price = 14999;
    } else if (slug?.[0] === "2N-3D") {
        data.durationtext = "3 Days & 2 Nights";
        data.tripdetails = "2N/3D Twin Sharing Dome Tent Stay + Complimentary Breakfast + Campsite Facilities";
        data.checkindate = "27-09-2025";
        data.price = 11999;
        data.p_price = 11999;
    }

    data.additionalTrips = [...additionalTrips];
