...
export async function getServerSideProps(context) {
  ...
  // New dynamic route params
  const pathSegments = context.params.name || []; // `name` will be the catch-all param (array)
      // e.g., "itineary"
  const itinearyId = pathSegments[1];  

  const carbonTraceLogin = async () => {
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_CARONTRACEURL}/login`,
        { username: 'admin@encampadventures.com', password: 'Encamp@#2024' },
        {
          headers: {
            'Accept-Encoding': 'compress',
            'Accept': 'application/json, text/plain, */*',
  ...
  const carbonRegisteraccess = await carbonTraceLogin();

  try {
    const res = await adminapi.get(`/getmaskitinerary/${itinearyId}`);
    const namecv = res?.data?.itineraryindex?.[0]?.nameite;
    const nameurl = namecv ? namecv.replace(/\s+/g, '-').toLowerCase() : null;
