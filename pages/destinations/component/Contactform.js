...
const Contactform = (props) => {
  ...
  const { AOS, toggle, setToggle, nameite, sendMail, Tagmanageri, state } = props;
  ...
  const submitAll = async (submitData) => {
    ...
    if (exc.message !== "" || exc.email !== "" || exc.contact !== "" || exc.firstName !== "" || exc.lastName !== "") {
      ...
      if (dataSubmitted && dataSubmitted.success === true) {
        ...
        let maildata = { subject: `Enquiry for Destination ${state}`, text: message };
        ...
        Tagmanageri([{ pagename: `destination-${state}` }], 'new_enquiry');
        resetParams();
        setBloading(false);
